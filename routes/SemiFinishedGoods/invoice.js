...
router.post("/", async (req, res) => {
  try {
    ...
    let embIssNoText = `SFG-${semiFinishedGoodsContextUnitData.lastEmbNo}-U${semiFinishedGoodsContextUnitData.embUnitData[0].embUnitVal}-23-24`;
...
const filteredString = filteredValues.join(',');
    ...
    try {
      ...
      const insertHeaderQuery =
        "INSERT INTO sfg_header (master_id, alter_id, vch_no, vch_date, estimate_cs_vch_no, design_no, lot_no, unit_no, narration) VALUES (?,?,?,?,?,?,?,?,?)";
    ...
    JSON.stringify(semiFinishedGoodsContextUnitData.lotNoVal) || 'NA',
      ...
      const insertDestinationQuery =
        "INSERT INTO sfg_destination (master_id, alter_id, vch_no, vch_date, item_name, design_no, godown, size,lot_no,quantity,rate, amt) VALUES (?,?, ?,?, ?, ?, ?, ?, ?,?,?,?)";
      if (
      ...
      const insertSourceQuery =
        "INSERT INTO sfg_source  (master_id, alter_id, vch_no, vch_date, item_name, design_no, godown,size,lot_no,quantity,rate, amt) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)";
        if (
