...
router.post("/", async (req, res) => {
  try {
    ...
    let embIssNoText = `PR-I-${printingIssContextUnitData.lastEmbNo}-U${printingIssContextUnitData.embUnitData[0].embUnitVal}-23-24`;
...
const filteredString = filteredValues.join(',');
    ...
    try {
      ...
      const insertHeaderQuery =
        "INSERT INTO printing_iss_header (master_id, alter_id, vch_no, estimate_cs_vch_no, vch_date, jobber_id, jobber_name, design_no, lot_no, size, unit_no, narration) VALUES (?,?, ?,?, ?, ?, ?,?, ?, ?, ?, ?)";
      ...
      await connection.query(insertHeaderQuery, [
        ...
        printingIssContextUnitData.embUnitData[0].unitNo || 'NA',
      ...
      const insertDestinationQuery =
        "INSERT INTO printing_iss_destination (master_id, alter_id, vch_no, vch_date, item_name, design_no, godown, size,lot_no,quantity,rate, amt) VALUES (?,?, ?,?, ?, ?, ?, ?, ?,?,?,?)";
      if (
      ...
      const insertSourceQuery =
        "INSERT INTO printing_iss_source  (master_id, alter_id, vch_no, vch_date, item_name, design_no, godown,type, size,lot_no,quantity,rate, amt,jw_rate, jw_amt) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
        if (
